...
app.post("/v1/webhook", (req, res, next) => {
	...
	var options = {
				method: "POST",
				uri: `https://maker.ifttt.com/trigger/notification/with/key/${config.iftttKey}`,
				body: {	value1: req.body.search_name },
				json: true
			};
			http(options).then(()=> {console.log('Notification Triggered: '+ req.body.search_name + ' @ ' + new Date().toUTCString())})
